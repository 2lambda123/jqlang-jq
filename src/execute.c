...
static int frame_size(struct bytecode *bc) {
  return sizeof(struct frame) +
         sizeof(union frame_entry) * (bc->nclosures + bc->nlocals);
}
...
static struct closure make_closure(struct jq_state *jq, uint16_t *pc) {
  ...
  if (idx & ARG_NEWCLOSURE) {
    ...
    struct closure cl = {fr->bc->subfunctions[subfn_idx], fridx};
    return cl;
  } else {
...
static struct frame *frame_push(struct jq_state *jq, struct closure callee,
  ...
  stack_ptr new_frame_idx =
      stack_push_block(&jq->stk, jq->curr_frame, frame_size(callee.bc));
...
static void frame_pop(struct jq_state *jq) {
  ...
  jq->curr_frame =
      stack_pop_block(&jq->stk, jq->curr_frame, frame_size(fp->bc));
}
...
void stack_save(jq_state *jq, uint16_t *retaddr, struct stack_pos sp) {
  jq->fork_top =
      stack_push_block(&jq->stk, jq->fork_top, sizeof(struct forkpoint));
...
uint16_t *stack_restore(jq_state *jq) {
  ...
  jq->fork_top =
      stack_pop_block(&jq->stk, jq->fork_top, sizeof(struct forkpoint));
  return retaddr;
}
  ...
  while (1) {
    ...
    case INSERT: {
      ...
      if (jv_get_kind(k) == JV_KIND_STRING) {
        ...
        char errbuf[15];
        set_error(
            jq, jv_invalid_with_msg(jv_string_fmt(
    ...
    case RANGE: {
      ...
      if (jv_get_kind(*var) != JV_KIND_NUMBER ||
        ...
        set_error(jq, jv_invalid_with_msg(
                          jv_string_fmt("Range bounds must be numeric")));
        jv_free(max);
        goto do_backtrack;
    ...
    case PATH_END: {
      ...
      if (!path_intact(jq, jv_copy(v))) {
        char errbuf[30];
        jv msg = jv_string_fmt("Invalid path expression with result %s",
      ...
      if (jv_get_kind(container) == JV_KIND_ARRAY) {
        ...
        if (opcode == EACH) {
          ...
          set_error(jq, jv_invalid_with_msg(jv_string_fmt(
                            ...
                            jv_dump_string_trunc(jv_copy(container), errbuf,
                                                 sizeof(errbuf)))));
        }
        keep_going = 0;
      }
    ...
    case ON_BACKTRACK(DESTRUCTURE_ALT): {
      if (jv_is_valid(jq->error)) {
        // `try EXP ...` backtracked here (no value, `empty`), so we backtrack
        // more
        jv_free(stack_pop(jq));
        goto do_backtrack;
      }
      ...
      if (opcode != ON_BACKTRACK(DESTRUCTURE_ALT)) {
        ...
        stack_push(jq,
                   jv_invalid_get_msg(jq->error)); // push the error's message
      } else {
        jv_free(jq->error);
      }
      jq->error = jv_null();
    ...
    case CALL_BUILTIN: {
      ...
      struct cfunction *function =
          &frame_current(jq)->bc->globals->cfunctions[*pc++];
      ...
      case 1:
        top = ((func_1)function->fptr)(jq, in[0]);
        break;
      case 2:
        top = ((func_2)function->fptr)(jq, in[0], in[1]);
        break;
      case 3:
        top = ((func_3)function->fptr)(jq, in[0], in[1], in[2]);
        break;
      case 4:
        top = ((func_4)function->fptr)(jq, in[0], in[1], in[2], in[3]);
        break;
      case 5:
        top = ((func_5)function->fptr)(jq, in[0], in[1], in[2], in[3], in[4]);
        break;
      ...
      default:
        ...
        return jv_invalid_with_msg(
            jv_string("Function takes too many arguments"));
      }
...
jv jq_format_error(jv msg) {
  if (jv_get_kind(msg) == JV_KIND_NULL ||
      (jv_get_kind(msg) == JV_KIND_INVALID &&
       !jv_invalid_has_msg(jv_copy(msg)))) {
    jv_free(msg);
...
void jq_set_nomem_handler(jq_state *jq, void (*nomem_handler)(void *),
                          void *data) {
...
static jv args2obj(jv args) {
  ...
  jv_array_foreach(args, i, v) r =
      jv_object_set(r, jv_object_get(jv_copy(v), kk), jv_object_get(v, vk));
  jv_free(args);
  jv_free(kk);
  jv_free(vk);
  return r;
...
int jq_compile_args(jq_state *jq, const char *str, jv args) {
  jv_nomem_handler(jq->nomem_handler, jq->nomem_handler_data);
  assert(jv_get_kind(args) == JV_KIND_ARRAY ||
         jv_get_kind(args) == JV_KIND_OBJECT);
  ...
  if (nerrors == 0) {
    ...
    if (nerrors == 0) {
      nerrors =
          block_compile(program, &jq->bc, locations, args = args2obj(args));
    }
  } else
    jv_free(args);
  if (nerrors)
    jq_report_error(jq, jv_string_fmt("jq: %d compile %s", nerrors,
...
int jq_halted(jq_state *jq) { return jq->halted; }

jv jq_get_exit_code(jq_state *jq) { return jv_copy(jq->exit_code); }

jv jq_get_error_message(jq_state *jq) { return jv_copy(jq->error_message); }
